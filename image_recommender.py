...
# -----------------------------
# Removed extract_features_safe as we get raw bytes/arrays from API
...
def get_dataset_features(data):
    ...
    try:
        if os.path.exists("text_embeddings_cache.npy"):
            ...
            # ideally we should save indices too, but for now we assume 1:1 mapping if counts match
            if len(all_features) == len(data):
                return all_features, data.index.tolist()
            else:
                # Fallback or partial? safe to just return empty to avoid mismatch
                print("Warning: Content mismatch between cache and live data.")
                # We can try to take min length
